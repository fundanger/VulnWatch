...
class ProfilesScreen(Screen):
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns("Name", "Min Severity", "Recipients", "Webhook")
        self._refresh_table()
...
class HistoryScreen(Screen):
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns("Started", "Finished", "Keywords", "New CVEs", "Emailed", "Error")
        self.action_refresh()
...
class BrowserScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="browser-controls"):
            yield Select(
                options=[],
                prompt="Select keyword table...",
                id="browser-table-select",
            )
        ...
        yield DataTable(id="browser-results")
    ...
    def _do_search(self) -> None:
        ...
        rows = database.query_cves(str(table_sel.value), search=search, min_severity=str(min_sev))
        ...
        self.query_one("#browser-status", Static).update(f"{len(rows)} result(s)")
    ...
    def _export(self, fmt: str) -> None:
        ...
        self.notify(f"Exported {n} rows to {out_path}", severity="information")
...
class MainScreen(Screen):
    ...
    def on_mount(self) -> None:
        ...
        self.query_one("#log", RichLog).write(
            ...
            "Use [bold]K[/bold] to manage keywords, [bold]H[/bold] for scan history, "
            "[bold]B[/bold] to browse collected CVEs."
        )
...
class SchedulerScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        yield Header(show_clock=True)
        yield Label("Windows Task Scheduler", id="sched-title")
        yield Static(
            "Register CVE Emailer as a background Task Scheduler job so it runs "
            "automatically even when this app is closed. Uses the checkFrequency "
            "from config.ini to set the repeat interval.",
            id="sched-hint",
        )
...
class CVEEmailerApp(App):
    ...
    #history-title, #browser-title, #sched-title {
        text-style: bold;
        text-align: center;
        color: $accent;
        margin: 1 0;
    ...
    #profiles-table { height: 12; border: solid $primary; margin-bottom: 1; }
    ...
    /* Browser */
    #browser-controls { height: auto; padding: 0 4; margin-bottom: 1; }
    #browser-controls Input { width: 1fr; margin-right: 1; }
    #browser-controls Select { width: 28; margin-right: 1; }
